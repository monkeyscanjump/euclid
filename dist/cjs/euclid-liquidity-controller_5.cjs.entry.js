...
var swap_store = require('./swap.store-Bi5XhaPy.js');
...
const EuclidLiquidityController = class {
    ...
    async initialize() {
        ...
        constants.liquidityStore.onChange('selectedPool', () => this.handlePoolChange());
        this.isInitialized = true;
        console.log('✅ Liquidity Controller initialized');
    }
    handlePoolChange() {
        const { selectedPool } = constants.liquidityStore.state;
        if (selectedPool) {
    ...
    async executeAddLiquidity() {
        try {
            const { selectedPool, token1, token2, token1Amount, token2Amount } = constants.liquidityStore.state;
            ...
            const token1Wallet = wallet_store.walletStore.getWallet(token1.chainUID);
            const token2Wallet = wallet_store.walletStore.getWallet(token2.chainUID);
            ...
            if (!wallet_store.walletStore.hasSufficientBalance(token1.chainUID, token1.id, token1Amount)) {
                return { success: false, error: `Insufficient ${token1.symbol} balance` };
            }
            if (!wallet_store.walletStore.hasSufficientBalance(token2.chainUID, token2.id, token2Amount)) {
                return { success: false, error: `Insufficient ${token2.symbol} balance` };
            }
            constants.liquidityStore.setAddingLiquidity(true);
            ...
            const result = await constants.apiClient.createAddLiquidityTransactionWrapped({
            ...
            if (result.success && result.data) {
                ...
                wallet_store.walletStore.addTransaction(primaryChain, {
                    txHash: txHash || result.data?.transactionHash || 'pending',
                    timestamp: Date.now(),
                    type: 'add_liquidity'
                });
                // Emit global event for transaction tracking
                events.dispatchEuclidEvent(events.EUCLID_EVENTS.TRANSACTION.SUBMITTED, {
                    txHash,
    ...
    async executeRemoveLiquidity(poolId, lpTokenAmount) {
        try {
            ...
            const lpBalance = wallet_store.walletStore.getWalletBalance(token1.chain_uid, `lp-${poolId}`);
            if (!lpBalance || BigInt(lpBalance.amount) < BigInt(lpTokenAmount)) {
                return { success: false, error: 'Insufficient LP token balance' };
            }
            constants.liquidityStore.setRemovingLiquidity(true);
            console.log('💧 Executing remove liquidity...', {
                poolId,
                lpAmount: lpTokenAmount,
            });
            // Execute remove liquidity via API
            const result = await constants.apiClient.createRemoveLiquidityTransactionWrapped({
                slippage_tolerance_bps: 50, // 0.5% = 50 basis points
                timeout: (Math.floor(Date.now() / 1000) + 1200).toString(), // 20 minutes
            ...
            if (result.success && result.data) {
                ...
                wallet_store.walletStore.addTransaction(token1.chain_uid, {
                    txHash,
                    timestamp: Date.now(),
            ...
            constants.liquidityStore.setRemovingLiquidity(false);
        }
    }
    // Event listeners
    async handleAddLiquidityExecution() {
        ...
        // Emit result event
        if (result.success) {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.LIQUIDITY.ADD_SUCCESS, {
                txHash: result.txHash,
            });
        }
        else {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.LIQUIDITY.ADD_FAILED, {
                error: result.error,
            });
        }
    }
    async handleRemoveLiquidityExecution(event) {
        ...
        // Emit result event
        if (result.success) {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.LIQUIDITY.REMOVE_SUCCESS, {
                txHash: result.txHash,
            });
        }
        else {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.LIQUIDITY.REMOVE_FAILED, {
                error: result.error,
            });
        }
    }
...
const EuclidSwapController = class {
    ...
    async initialize() {
        ...
        swap_store.swapStore.onChange('fromToken', () => this.handleTokenChange());
        swap_store.swapStore.onChange('toToken', () => this.handleTokenChange());
        swap_store.swapStore.onChange('fromAmount', () => this.handleAmountChange());
        this.isInitialized = true;
        console.log('✅ Swap Controller initialized');
    }
    handleTokenChange() {
        // Clear existing routes when tokens change
        swap_store.swapStore.setRoutes([]);
        swap_store.swapStore.setSelectedRoute(undefined);
        this.stopRoutePolling();
        // Start polling if we have both tokens and an amount
        if (swap_store.swapStore.state.fromToken && swap_store.swapStore.state.toToken && swap_store.swapStore.state.fromAmount) {
            this.startRoutePolling();
        }
    }
    handleAmountChange() {
        const { fromToken, toToken, fromAmount } = swap_store.swapStore.state;
    ...
    startRoutePolling() {
        ...
        this.routePollingInterval = window.setInterval(() => {
            this.fetchRoutes();
        }, constants.DEFAULTS.ROUTE_REFRESH_INTERVAL);
    }
    stopRoutePolling() {
        if (!this.routePollingActive)
            return;
    ...
    async fetchRoutes() {
        ...
        try {
            swap_store.swapStore.setLoadingRoutes(true);
            ...
            // Call the routes API
            const response = await constants.apiClient.getRoutesWrapped({
            ...
            if (response.success && response.data) {
                ...
                swap_store.swapStore.setRoutes(routePaths);
                // Auto-select the best route (first one, typically best by default)
                if (routePaths.length > 0 && !swap_store.swapStore.state.selectedRoute) {
                    swap_store.swapStore.setSelectedRoute(routePaths[0]);
                }
            ...
            swap_store.swapStore.setLoadingRoutes(false);
        }
    }
    getUserAddressForChain(chainUID) {
        const wallet = wallet_store.walletStore.getWallet(chainUID);
        return wallet?.address;
    }
    // Execute swap transaction
    async executeSwap() {
        try {
            const { fromToken, toToken, fromAmount, selectedRoute, slippage } = swap_store.swapStore.state;
            ...
            const wallet = wallet_store.walletStore.getWallet(fromToken.chainUID);
            if (!wallet?.isConnected) {
                return { success: false, error: `Wallet not connected for ${fromToken.chainUID}` };
            }
            // Check sufficient balance
            if (!wallet_store.walletStore.hasSufficientBalance(fromToken.chainUID, fromToken.id, fromAmount)) {
                return { success: false, error: 'Insufficient balance' };
            }
            swap_store.swapStore.setSwapping(true);
            ...
            const swapResult = await constants.apiClient.createSwapTransactionWrapped({
                amount_in: fromAmount,
                asset_in: {
                    token: fromToken.id,
            ...
            if (swapResult.success && swapResult.data) {
                ...
                // Add transaction to wallet store
                wallet_store.walletStore.addTransaction(fromToken.chainUID, {
                    txHash,
                    timestamp: Date.now(),
                    type: 'swap'
                });
            ...
            swap_store.swapStore.setSwapping(false);
        }
    }
    ...
    async handleSwapExecution() {
        ...
        // Emit result event
        if (result.success) {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.SWAP.EXECUTE_SUCCESS, {
                txHash: result.txHash,
            });
        }
        else {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.SWAP.EXECUTE_FAILED, {
                error: result.error,
            });
        }
    }
...
const EuclidTxTrackerController = class {
    ...
    async checkTransactionStatus(txHash, chainUID, type, currentPollCount = 0) {
        try {
            ...
            const response = await constants.apiClient.trackTransactionWrapped(txHash, chainUID);
            if (response.success && response.data) {
                const { status } = response.data;
                // Update transaction status in wallet store
                wallet_store.walletStore.updateTransactionStatus(chainUID, txHash, status);
                if (status === 'confirmed' || status === 'failed') {
                    // Transaction is final, stop tracking
                    this.trackingTransactions.delete(txHash);
                    // Emit global event with final status
                    events.dispatchEuclidEvent(events.EUCLID_EVENTS.TRANSACTION.FINALIZED, {
                        txHash,
                        chainUID,
                        type,
                        status,
                    ...
                    if (newPollCount >= 120) {
                        ...
                        wallet_store.walletStore.updateTransactionStatus(chainUID, txHash, 'failed');
                        events.dispatchEuclidEvent(events.EUCLID_EVENTS.TRANSACTION.TIMEOUT, {
                            txHash,
                            chainUID,
                            type
                        });
    ...
    refreshUserDataAfterSuccess(chainUID, type) {
        ...
        events.dispatchEuclidEvent(events.EUCLID_EVENTS.USER.REFRESH_DATA);
        if (type === 'add_liquidity' || type === 'remove_liquidity') {
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.LIQUIDITY.POSITIONS_REFRESH);
        }
        // Refresh balances for the affected chain
        events.dispatchEuclidEvent(events.EUCLID_EVENTS.USER.BALANCES_REFRESH, { chainUID });
    }
    ...
    handleGetTrackingStats() {
        ...
        events.dispatchEuclidEvent(events.EUCLID_EVENTS.TRANSACTION.STATS_RESPONSE, stats);
    }
    onInitializedChange(newValue) {
        if (newValue) {
...
const EuclidUserDataController = class {
    ...
    disconnectedCallback() {
        ...
        wallet_store.walletStore.dispose?.();
    }
    async initialize() {
        console.log('👤 Initializing User Data Controller...');
        // Listen for wallet connection changes
        wallet_store.walletStore.onChange('wallets', async (wallets) => {
    ...
    handleWalletDisconnection() {
        ...
        wallet_store.walletStore.clear();
    }
    async loadUserBalances() {
        ...
        for (const wallet of connectedWallets) {
            try {
                ...
                const balanceResponse = await constants.apiClient.getBalance(wallet.address, wallet.chainUID);
                if (balanceResponse.success && balanceResponse.data) {
                    ...
                    if (balanceData.balance?.all) {
                        ...
                        wallet_store.walletStore.updateWalletBalances(wallet.chainUID, chainBalances);
                    }
                }
            }
            catch (error) {
    ...
    setupPeriodicRefresh() {
        ...
        }, constants.DEFAULTS.BALANCE_REFRESH_INTERVAL);
    }
    ...
    onInitializedChange(newValue) {
        if (newValue) {
            ...
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.USER.CONTROLLER_READY, {
                timestamp: Date.now()
            });
        }
    }
...
const EuclidWalletController = class {
    ...
    disconnectEvmWallets() {
        ...
        evmChains.forEach(chainUID => {
            wallet_store.walletStore.removeWallet(chainUID);
        });
    }
    async handleWalletConnectionRequest(event) {
        ...
        try {
            ...
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.WALLET.CONNECT_FAILED, {
                chainUID,
                walletType,
                error: error.message
            });
    ...
    handleWalletDisconnectionRequest(event) {
        ...
        wallet_store.walletStore.disconnectWallet(chainUID);
        // Emit disconnection success event
        events.dispatchEuclidEvent(events.EUCLID_EVENTS.WALLET.DISCONNECT_SUCCESS, { chainUID });
    }
    async connectWallet(chainUID, walletType) {
        // Get chain configuration
        const chainConfig = constants.marketStore.getChain(chainUID);
        if (!chainConfig) {
        ...
        if (result.success && result.address) {
            // Add wallet to store
            wallet_store.walletStore.addWallet(chainUID, {
            ...
            events.dispatchEuclidEvent(events.EUCLID_EVENTS.WALLET.CONNECT_SUCCESS, {
                chainUID,
                walletType,
                address: result.address
            });
...
exports.euclid_liquidity_controller = EuclidLiquidityController;
exports.euclid_swap_controller = EuclidSwapController;
exports.euclid_tx_tracker_controller = EuclidTxTrackerController;
exports.euclid_user_data_controller = EuclidUserDataController;
exports.euclid_wallet_controller = EuclidWalletController;
//# sourceMappingURL=euclid-liquidity-controller.euclid-swap-controller.euclid-tx-tracker-controller.euclid-user-data-controller.euclid-wallet-controller.entry.cjs.js.map
